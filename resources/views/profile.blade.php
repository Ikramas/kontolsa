                ...
                <div class="zSidebar-fixed">
                    <ul class="zSidebar-menu" id="sidebarMenu">
                        ...
                        <li class="active"> {{-- Menandai menu Profil sebagai aktif --}}
                            <a href="{{ url('/profile') }}" class="d-flex align-items-center cg-10 active">
                                <div class="d-flex">
                ...
                <div class="p-30">
                    ...
                    <div class="bg-white bd-half bd-c-ebedf0 bd-ra-25 p-30">
                        <ul class="zList-one">
                            <li>
                                <p>Nama</p>
                                <p>{{ $user->name ?? 'Tidak ada data' }}</p>
                            </li>
                            <li>
                                <p>Email</p>
                                <p><a href="mailto:{{ $user->email ?? '' }}" class="text-707070">{{ $user->email ?? 'Tidak ada data' }}</a></p>
                            </li>
                            {{-- Menggunakan $user->mobile dari model User.php --}}
                            <li>
                                <p>Phone</p>
                                <p>{{ $user->mobile ?? 'Tidak ada data' }}</p>
                            </li>
                            
                            {{-- BAGIAN DATA PENANGGUNG JAWAB / PERUSAHAAN (DARI MODEL COMPANY) --}}
                            {{-- Hanya tampil jika objek company ada --}}
                            @if ($user->company)
                                <li>
                                    <p>Nomor Induk Kependudukan (NIK)</p>
                                    <p>{{ $user->company->pic_nik ?? 'Tidak ada data' }}</p>
                                </li>
                                <li>
                                    <p>Nomor Pokok Wajib Pajak (NPWP)</p>
                                    <p>{{ $user->company->pic_npwp ?? 'Tidak ada data' }}</p>
                                </li>
                                <li>
                                    <p>Jabatan</p>
                                    <p>{{ $user->company->pic_position ?? 'Tidak ada data' }}</p>
                                </li>
                                <li>
                                    <p>Alamat</p>
                                    <p>{{ $user->company->pic_address ?? 'Tidak ada data' }}</p>
                                </li>

                                {{-- BAGIAN INI HANYA MUNCUL JIKA STATUS PERUSAHAAN ADALAH 'Aktif' --}}
                                @if ($user->company->membership_status == 'Aktif')
                                    <li>
                                        <p>Nomor Registrasi Nasional</p>
                                        <p>{{ $user->company->national_registration_number ?? 'Tidak ada data' }}</p>
                                    </li>
                                    <li>
                                        <p>Nomor Anggota</p>
                                        <p>{{ $user->company->member_number ?? 'Tidak ada data' }}</p>
                                    </li>
                                    {{-- PERBAIKAN: Tambahkan Tanggal Expired --}}
                                    <li>
                                        <p>Tanggal Expired</p>
                                        <p>
                                            @if ($user->company->membership_expired_date)
                                                {{ \Carbon\Carbon::parse($user->company->membership_expired_date)->translatedFormat('d F Y') }}
                                            @else
                                                Tidak ada data
                                            @endif
                                        </p>
                                    </li>
                                    {{-- PERBAIKAN: Tambahkan Status Keanggotaan dengan desain warna --}}
                                    <li>
                                        <p>Status</p>
                                        <p>
                                            @php
                                            ...
                                            @endphp
                                            <span class="d-inline-block py-6 px-10 bd-ra-6 fs-14 fw-500 lh-16 {{ $class }}">
                                                {{ $status }}
                                            </span>
                                        </p>
                                    </li>
                                    {{-- PERBAIKAN: Tambahkan indikator grup tertutup --}}
                                    <li>
                                        <p>Grup Anggota</p>
                                        <p>
                                ...
                                @endif
                            @else
                                {{-- Pesan opsional jika user belum memiliki data perusahaan --}}
                                <li>
