...
<head>
    ...
    <title>Daftar Transaksi - Kadin Indonesia</title>
...
<body class="direction-ltr">
            ...
            <div class="zSidebar">
                ...
                <div class="zSidebar-fixed">
                    <ul class="zSidebar-menu" id="sidebarMenu">
                        ...
                        <li class="active"> {{-- Menandai menu Transaksi sebagai aktif --}}
                            <a href="{{ url('/transactions') }}" class="d-flex align-items-center cg-10 active">
                                <div class="d-flex">
            ...
            <div class="zMainContent">
                ...
                <div class="p-30">
                    <div class="d-flex flex-wrap justify-content-between align-items-center pb-16">
                        <h4 class="fs-24 fw-500 lh-34 text-black">Daftar Transaksi</h4>
                    </div>

                    <div class="bg-white bd-half bd-c-ebedf0 bd-ra-25 p-30">
                        <div class="">
                            <table class="table table-striped zTable" id="transaction-table" width="100%">
                                <thead>
                                    <tr>
                                        <th title="No" width="10">No</th>
                                        <th title="Nama">Nama</th>
                                        <th title="Tipe Keanggotaan">Tipe Keanggotaan</th>
                                        <th title="Klasifikasi">Klasifikasi</th>
                                        <th title="Jumlah">Jumlah</th>
                                        <th title="Tanggal">Tanggal</th>
                                        <th title="Jenis Pembayaran">Jenis Pembayaran</th>
    ...
    {{-- Memastikan summernote.min.js ada di folder assets/js/ --}}
    ...
    <script src="{{ asset('assets/js/common.js?ver=0') }}"></script> {{-- Path corrected --}}

    <script type="text/javascript">
    document.addEventListener("DOMContentLoaded", function() {
        window.LaravelDataTables = window.LaravelDataTables || {};
        window.LaravelDataTables["transaction-table"] = $("#transaction-table").DataTable({
            "serverSide": true,
            "processing": true,
            "ajax": {
            ...
            "columns": [
                ...
                { "data": "user_name", "name": "user_name", "title": "Nama", "orderable": true, "searchable": true },
                { "data": "membership_type", "name": "membership_type", "title": "Tipe Keanggotaan", "orderable": true, "searchable": true },
                { "data": "membership_classification", "name": "membership_classification", "title": "Klasifikasi", "orderable": true, "searchable": true },
                { "data": "amount", "name": "amount", "title": "Jumlah", "orderable": true, "searchable": true,
                    "render": function(data, type, row) {
                        if (type === 'display' || type === 'filter') {
                            return currencyPlacement === 'before' ? currencySymbol + ' ' + parseFloat(data).toLocaleString('id-ID') : parseFloat(data).toLocaleString('id-ID') + ' ' + currencySymbol;
                        }
                        return data;
                    }
                },
                {
                    ...
                    "render": function(data, type, row) {
                        ...
                        return row.created_at_raw || data;
                    }
                },
                { "data": "payment_type", "name": "payment_type", "title": "Jenis Pembayaran", "orderable": true, "searchable": true },
                { "data": "payment_status", "name": "payment_status", "title": "Status", "orderable": true, "searchable": true },
                { "data": "action", "name": "action", "title": "Aksi", "orderable": false, "searchable": false, "width": 60, "className": "text-md-center" }
            ],
            "responsive":true,
            "order":[[5,"desc"]],
        ...
        $('#transaction-table').on('click', '.btn-show-invoice-modal', function(e) {
            e.preventDefault();
            const rowData = window.LaravelDataTables["transaction-table"].row($(this).parents('tr')).data();

            const printInvoiceUrl = rowData.action_print_url;

            const logoUrl = "{{ asset('assets/uploads/Setting/6281729670913.png') }}";
            const invoiceNumber = rowData.transaction_uuid || rowData.id || 'N/A';

            let paymentStatusText = $(rowData.payment_status).text().trim();
            let headerStatusClass = '';

            // Update status text for the modal
            if (paymentStatusText === 'Sukses') {
                paymentStatusText = 'Paid'; // Change "Sukses" to "Paid"
                headerStatusClass = 'text-0fa958 bg-0fa958-10';
            } else if (paymentStatusText === 'Pending') {
                // Keep "Pending" as is
                headerStatusClass = 'text-f5b40a bg-f5b40a-10';
            } else {
                headerStatusClass = 'text-secondary bg-ededed';
            }

            const customerName = rowData.user_name ?? 'N/A';
            const customerEmail = rowData.email ?? 'N/A';
            const companyName = rowData.company_name ?? 'N/A';

            const membershipType = rowData.membership_type || 'N/A';
            const classification = rowData.membership_classification || 'N/A';
            const duration = rowData.duration_qty ? rowData.duration_qty + ' Tahun' : '1 Tahun';
            const amount = rowData.amount || 0;

            const subTotal = amount;
            const totalAmount = amount;

            const transactionDate = rowData.created_at || 'N/A';
            const paymentTypeInModal = "Virtual Account"; // Static value as per previous request
            const transactionIdFromDb = rowData.transaction_uuid || rowData.id || 'N/A';

            const modalBodyContent = `
                <div class="bd-b-one bd-c-ededed mb-44 pb-25 d-flex justify-content-between align-items-center flex-wrap cg-15 rg-10">
                    <div>
                    </div>
                    <a href="${printInvoiceUrl}" target="_blank" class="btn btn-primary border-0 d-flex align-items-right cg-8 px-10 py-5 bd-ra-4">
                        <p class="fs-14 fw-500 lh-20">Print</p>
                ...
                <div id="printableArea">
                    <div class="d-flex justify-content-between align-items-center g-10 bd-b-one bd-c-ededed pb-33 mb-20">
                        <div>
                            <a href="#" class="d-block mb-6"><img style="height: 100px;" src="${logoUrl}" alt="Kadin Indonesia"></a>
                            <p class="fs-15 fw-500 lh-20 text-1b1c17">No. ${invoiceNumber}</p>
                        </div>
                        <p class="py-14 px-25 fs-14 fw-500 lh-20 ${headerStatusClass} bd-ra-4">${paymentStatusText}</p>
                    </div>
                    <div class="d-flex justify-content-between g-10 pb-22">
                        <div>
                            <h4 class="fs-18 fw-500 lh-22 text-1b1c17 pb-18">Invoice To</h4>
                            <h4 class="fs-18 fw-500 lh-22 text-707070 pb-5">${customerName}</h4>
                            <p class="fs-13 fw-400 lh-20 text-707070 pb-5">${customerEmail}</p>
                            <p class="fs-13 fw-400 lh-20 text-707070 pb-10">${companyName}</p>
                        </div>
                        <div class="text-end">
                            <h4 class="fs-18 fw-500 lh-22 text-1b1c17 pb-18">Pay To</h4>
                            <h4 class="fs-15 fw-500 lh-20 text-707070 pb-10">Kadin Indonesia</h4>
                            <p class="fs-14 fw-500 lh-20 text-707070">Jl. H. R. Rasuna Said Blok X-5 No.Kav. 2-3, Kuningan, Jakarta 12950</p>
                        </div>
                    </div>
                    <div class="pb-33">
                        <h4 class="fs-18 fw-500 lh-22 text-1b1c17 pb-20">Invoice Items</h4>
                        <div class="table-responsive">
                            <table class="table zTable zTable-2">
                                ...
                                <tbody>
                                    <tr>
                                        <td><p>${membershipType}</p></td>
                                        <td><p>${classification}</p></td>
                                        <td><p>${duration}</p></td>
                                        <td><p>${currencyPlacement === 'before' ? currencySymbol + ' ' : ''}${parseFloat(amount).toLocaleString('id-ID')}${currencyPlacement === 'after' ? ' ' + currencySymbol : ''}</p></td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td></td>
                                        <td>Sub Total</td>
                                        <td>${currencyPlacement === 'before' ? currencySymbol + ' ' : ''}${parseFloat(subTotal).toLocaleString('id-ID')}${currencyPlacement === 'after' ? ' ' + currencySymbol : ''}</td>
                                    </tr>
                                    <tr>
                                        <td></td>
                                        <td></td>
                                        <td>Total</td>
                                        <td>${currencyPlacement === 'before' ? currencySymbol + ' ' : ''}${parseFloat(totalAmount).toLocaleString('id-ID')}${currencyPlacement === 'after' ? ' ' + currencySymbol : ''}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                    </div>
                    <h4 class="fs-18 fw-500 lh-22 text-1b1c17 pb-20">Riwayat Transaksi</h4>
                    <div class="bg-fafafa bd-one bd-c-ededed bd-ra-4 table-responsive">
                        <table class="table zTable zTable-2 zTable-3">
                            ...
                            <tbody>
                                <tr>
                                    <td><p class="min-w-110">${transactionDate}</p></td>
                                    <td><p class="min-w-110">${paymentTypeInModal}</p></td>
                                    <td><p>${transactionIdFromDb}</p></td>
                                    <td><p>${currencyPlacement === 'before' ? currencySymbol + ' ' : ''}${parseFloat(amount).toLocaleString('id-ID')}${currencyPlacement === 'after' ? ' ' + currencySymbol : ''}</p></td>
                                </tr>
                            </tbody>
                        </table>
...
<script>
    ...
    var currencyPlacement = "before"; // 'before' or 'after'
</script>
</body>
</html>